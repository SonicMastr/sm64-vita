...
static bool gfx_vita_opengles2_z_is_from_0_to_1(void) {
    return false;
}

static void gfx_vita_opengles2_vertex_array_set_attribs(struct ShaderProgram *prg) {
...
static void gfx_vita_opengles2_set_uniforms(struct ShaderProgram *prg) {
    if (prg->used_noise) {
...
static void gfx_vita_opengles2_unload_shader(struct ShaderProgram *old_prg) {
    if (old_prg != NULL) {
...
static void gfx_vita_opengles2_load_shader(struct ShaderProgram *new_prg) {
    glUseProgram(new_prg->opengl_program_id);
    gfx_vita_opengles2_vertex_array_set_attribs(new_prg);
    gfx_vita_opengles2_set_uniforms(new_prg);
}
...
static struct ShaderProgram *gfx_vita_opengles2_create_and_load_new_shader(uint32_t shader_id) {
    ...
    if (cc_features.opt_alpha && cc_features.opt_noise) {
        append_line(fs_buf, &fs_len, "texel.a *= floor(random(float3(floor(gl_FragCoord.xy * (240.0 / float(window_height))), float(frame_count))) + 0.5);");
    }
    ...
    gfx_vita_opengles2_load_shader(prg);
...
static struct ShaderProgram *gfx_vita_opengles2_lookup_shader(uint32_t shader_id) {
...
static void gfx_vita_opengles2_shader_get_info(struct ShaderProgram *prg, uint8_t *num_inputs, bool used_textures[2]) {
    *num_inputs = prg->num_inputs;
    used_textures[0] = prg->used_textures[0];
    used_textures[1] = prg->used_textures[1];
}

static GLuint gfx_vita_opengles2_new_texture(void) {
    GLuint ret;
    glGenTextures(1, &ret);
    return ret;
}

static void gfx_vita_opengles2_select_texture(int tile, GLuint texture_id) {
    glActiveTexture(GL_TEXTURE0 + tile);
    glBindTexture(GL_TEXTURE_2D, texture_id);
}

static void gfx_vita_opengles2_upload_texture(const uint8_t *rgba32_buf, int width, int height) {
...
static void gfx_vita_opengles2_set_sampler_parameters(int tile, bool linear_filter, uint32_t cms, uint32_t cmt) {
    glActiveTexture(GL_TEXTURE0 + tile);
...
static void gfx_vita_opengles2_set_depth_test(bool depth_test) {
    if (depth_test) {
...
static void gfx_vita_opengles2_set_depth_mask(bool z_upd) {
    glDepthMask(z_upd ? GL_TRUE : GL_FALSE);
}

static void gfx_vita_opengles2_set_zmode_decal(bool zmode_decal) {
    if (zmode_decal) {
        glPolygonOffset(-2, -2);
...
static void gfx_vita_opengles2_set_viewport(int x, int y, int width, int height) {
    glViewport(x, y, width, height);
    current_height = height;
}

static void gfx_vita_opengles2_set_scissor(int x, int y, int width, int height) {
    glScissor(x, y, width, height);
}

static void gfx_vita_opengles2_set_use_alpha(bool use_alpha) {
    if (use_alpha) {
        glEnable(GL_BLEND);
    } else {
        glDisable(GL_BLEND);
...
static void gfx_vita_opengles2_draw_triangles(float buf_vbo[], size_t buf_vbo_len, size_t buf_vbo_num_tris) {
    //printf("flushing %d tris\n", buf_vbo_num_tris);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * buf_vbo_len, buf_vbo, GL_STREAM_DRAW);
    glDrawArrays(GL_TRIANGLES, 0, 3 * buf_vbo_num_tris);
}

static void gfx_vita_opengles2_init(void) {
#if FOR_WINDOWS
    glewInit();
#endif
...
static void gfx_vita_opengles2_on_resize(void) {
}

static void gfx_vita_opengles2_start_frame(void) {
    frame_count++;
...
static void gfx_vita_opengles2_end_frame(void) {
}

static void gfx_vita_opengles2_finish_render(void) {
}

struct GfxRenderingAPI gfx_vita_opengles2_api = {
    gfx_vita_opengles2_z_is_from_0_to_1,
    gfx_vita_opengles2_unload_shader,
    gfx_vita_opengles2_load_shader,
    gfx_vita_opengles2_create_and_load_new_shader,
    gfx_vita_opengles2_lookup_shader,
    gfx_vita_opengles2_shader_get_info,
    gfx_vita_opengles2_new_texture,
    gfx_vita_opengles2_select_texture,
    gfx_vita_opengles2_upload_texture,
    gfx_vita_opengles2_set_sampler_parameters,
    gfx_vita_opengles2_set_depth_test,
    gfx_vita_opengles2_set_depth_mask,
    gfx_vita_opengles2_set_zmode_decal,
    gfx_vita_opengles2_set_viewport,
    gfx_vita_opengles2_set_scissor,
    gfx_vita_opengles2_set_use_alpha,
    gfx_vita_opengles2_draw_triangles,
    gfx_vita_opengles2_init,
    gfx_vita_opengles2_on_resize,
    gfx_vita_opengles2_start_frame,
    gfx_vita_opengles2_end_frame,
    gfx_vita_opengles2_finish_render
};
